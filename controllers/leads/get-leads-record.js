...
const GetLeadsRecord = async ({ filter, sort, skip, limit, userId }) => {
  const { pageId, groupId, postId, keyword } = filter;
  ...
  if (keyword) {
    if (keyword.includes(" ")) {
      const regex = new RegExp(
        keyword.replace(/(\S+)/g, (s) => `\\b${s}.*`).replace(/\s+/g, "")
      );
      extend(selector, {
        $or: [
  ...
  const leadsRecord = await Lead.find(selector)
    .sort(sort)
    .skip(skip)
    .limit(limit);
